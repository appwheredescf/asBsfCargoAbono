			...
			if (!jsonRes.equals("")) {
				...
				if (resGral.getESTATUS().equals("0")) {
					...
					if(resGral.getCODIGO().trim().equals("TCB08543") || resGral.getCODIGO().trim().equals("TCB08516")
						...
						CatalogoErrores catalogoErrores = new CatalogoErrores();
						catalogoErrores = (CatalogoErrores) this.util.jsonToObject(catalogoErrores, jsonRes, nodos);
						mensajeError = this.util.limpiarErrores(catalogoErrores.getSTD_TRN_MSJ_PARM_V());
						response.setDescripcion(resGral.getMENSAJE().trim());
						response.setStatus(-1);
					}
				}
