...
import com.bansefi.nss.cargoabono.vo.CatalogoErrores;
import com.bansefi.nss.cargoabono.vo.CodigoRetorno;
import com.bansefi.nss.cargoabono.vo.DatosConsultaSaldo;
...
import com.bansefi.nss.cargoabono.vo.SaldoBloqueos;
import com.bansefi.nss.cargoabono.vo.SaldoConectados;
import com.bansefi.nss.cargoabono.vo.SaldoContable;
import com.bansefi.nss.cargoabono.vo.SaldoDisponible;
import com.bansefi.nss.cargoabono.vo.SaldoIncidencias;
import com.bansefi.nss.cargoabono.vo.SaldoRetenido;
...
public class PasivosAcuerdosServices {
	...
	public ResponseService ConsultaClabe(String acuerdo, String entidad, String terminal) {
		...
		try {
			...
			if (salida.length() > 0) {
				...
				JSONObject jsonResponse = jsonSalida.getJSONObject("ConsultaCLABEResponse").getJSONObject("return")
	...
	public ResponseConsultaSaldo ConsultaSaldoAnterior(String acuerdo, String entidad, String terminal) {
		...
		try {
			...
			if (salida.length() > 0) {
				JSONObject jsonSalida = new JSONObject(salida);
				JSONObject jsonResponse = jsonSalida.getJSONObject("ConsultaSaldosResponse").getJSONObject("return")
						.getJSONObject("TR_CONS_SALDOS_VISTA_TRN").getJSONObject("TR_CONS_SALDOS_VISTA_TRN_O");
				...
				if (status == 1) {
					...
					errores = jsonErrores != null ? jsonErrores.toString()
							: jsonResponse.optJSONArray("errores") != null
									? jsonResponse.optJSONArray("errores").toString() : "";
	...
	public ResponseUltimaTransaccion UltimaTransaccion(String acuerdo, String entidad, String terminal,
			String fechaActual) {
		EndpointProperties endpointProperties = new EndpointProperties();
		...
		try {
			...
			if (salida.length() > 0) {
				...
				JSONObject jsonResponse = jsonSalida.getJSONObject("ConsultaUltimaTransaccionResponse")
						.getJSONObject("return").getJSONObject("RESPONSE");
				...
				if (status == 1) {
					...
					try {
						JSONArray jsonApuntesArra = jsonResponse.getJSONObject("DEPOSITOS")
								.getJSONArray("AF_APNTE_AUDIT_V");
					...
					if (isArray == false) {
						JSONObject jsonResponseApunt = jsonResponse.getJSONObject("DEPOSITOS")
								.getJSONObject("AF_APNTE_AUDIT_V");
						ArrayList<AF_APNTE_AUDIT_V> apuntes = new ArrayList<ResponseUltimaTransaccion.AF_APNTE_AUDIT_V>();
	...
	public ResponseServiceCargoAbono CargoAbono(String entidad, String acuerdo, String tipoOp, String fechaValor,
			String impNom, String clopSis, String subClop, String cajaInt, String concepto, String fechaOperacion,
			String idOrigenApnt, String sucursal, String idTerminal, String idEmpleado, String horaOp, String cotTx) {
		...
		try {
			...
			if (salida.length() > 0) {
				JSONObject jsonSalida = new JSONObject(salida);
				JSONObject jsonResponse = jsonSalida.getJSONObject("CargoAbonoResponse").getJSONObject("return")
						.getJSONObject("RESPONSE");
				Integer status = jsonResponse.getInt("STATUS");
				if (status == 1) {
					response.setStatus(1);
	...
	public ResponseServiceObject ConsultaIdMovCargoAbono(String IdMovimiento) {
		...
		try {
			...
			if (salida.length() > 0) {
				JSONObject jsonSalida = new JSONObject(salida);
				JSONObject jsonResponse = jsonSalida.getJSONObject("ObtieneMovimientoCargoAbonoResp")
						.getJSONObject("MovimientoCargoAbono");
				MovimientoCargoAbono oResCA = new MovimientoCargoAbono();
				try {
					oResCA = (MovimientoCargoAbono) this.utilJson.jsonToObject(oResCA, jsonResponse.toString(),
	...
	public EntResponRegCargAbono RegistraCargoAbono(EntMovCargoAbono oMovCargAbon) {
		...
		try {
			...
			if (salida.length() > 0) {
				JSONObject jsonSalida = new JSONObject(salida);
				JSONObject jsonResponse = jsonSalida.getJSONObject("RegistraMovimientosCargoAbonoResp")
						.getJSONObject("MovimientosCargoAbono");
	...
	public ResponseService ActStatusMovCargoAbono(ActualizaStatusMovimientoCargoAbono oEntStat) {
		...
		try {
			...
			if (salida.length() > 0) {
				JSONObject jsonSalida = new JSONObject(salida);
				JSONObject jsonResponse = jsonSalida.getJSONObject("ActualizaStatusMovimientoCargoAbonoResp")
						.getJSONObject("StatusMovimientoCargoAbono");
	...
	/*
	 * Metodo que consume los servicios de consulta de saldo de la UANAL
	 */
	@SuppressWarnings("unchecked")
	public ResponseConsultaSaldo ConsultaSaldo(RequestConsultaSaldo request) {
		// Instancia del clase properties para tomar ruta de regla que se
		// consumira
		...
		try {
			String jsonRes = util.callRestPost(request, pProp.getSALDO_UANAL());
			if (!jsonRes.equals("")) {
				ArrayList<String> nodos = new ArrayList<String>();
				nodos.add("EjecutarResponse");
				nodos.add("EjecutarResult");
				resGral = (ResGeneral) util.jsonToObject(resGral, jsonRes, nodos);
				if (resGral.getESTATUS().equals("0")) {
					nodos.add("ResponseBansefi");
					nodos.add("OTR_CONS_SALDOS_VISTA_TRN");
					codigoRetorno = (CodigoRetorno) util.jsonToObject(codigoRetorno, jsonRes, nodos);
					if (codigoRetorno.getRTRN_CD().equals("0001")) {
						nodos.add("TR_CONS_SALDOS_VISTA_EVT");
						DatosConsultaSaldo datosConsultaSaldo = new DatosConsultaSaldo();
						datosConsultaSaldo = (DatosConsultaSaldo) util.jsonToObject(datosConsultaSaldo, jsonRes, nodos);
						nodos.add("PSV_SDO_CONTABLE_V");
						SaldoContable saldoContable = new SaldoContable();
						saldoContable = (SaldoContable) util.jsonToObject(saldoContable, jsonRes, nodos);
						nodos.remove("PSV_SDO_CONTABLE_V");
						nodos.add("PSV_SDO_RETEN_V");
						SaldoRetenido saldoRetenido = new SaldoRetenido();
						saldoRetenido = (SaldoRetenido) util.jsonToObject(saldoRetenido, jsonRes, nodos);
						nodos.remove("PSV_SDO_RETEN_V");
						nodos.add("PSV_SDO_AUT_V");
						SaldoAutorizado saldoAutorizado = new SaldoAutorizado();
						saldoAutorizado = (SaldoAutorizado) util.jsonToObject(saldoAutorizado, jsonRes, nodos);
						nodos.remove("PSV_SDO_AUT_V");
						nodos.add("PSV_DISPO_V");
						SaldoDisponible saldoDisponible = new SaldoDisponible();
						saldoDisponible = (SaldoDisponible) util.jsonToObject(saldoDisponible, jsonRes, nodos);
						nodos.remove("PSV_DISPO_V");
						nodos.add("PSV_SDO_INCID_V");
						SaldoIncidencias saldoIncidencias = new SaldoIncidencias();
						saldoIncidencias = (SaldoIncidencias) util.jsonToObject(saldoIncidencias, jsonRes, nodos);
						nodos.remove("PSV_SDO_INCID_V");
						nodos.add("PSV_SDO_CONECT_V");
						SaldoConectados saldoConectados = new SaldoConectados();
						saldoConectados = (SaldoConectados) util.jsonToObject(saldoConectados, jsonRes, nodos);
						nodos.remove("PSV_SDO_CONECT_V");
						nodos.add("PSV_BLOQUEOS_V");
						SaldoBloqueos bloqueos = new SaldoBloqueos();
						bloqueos = (SaldoBloqueos) util.jsonToObject(bloqueos, jsonRes, nodos);
						// Seteo de datos al response
						response.setSDO_CONTABLE(
								(saldoContable.getIMP_SDO().equals(null)) ? "" : saldoContable.getIMP_SDO());
						response.setSDO_RETEN(
								(saldoRetenido.getIMP_SDO().equals(null)) ? "" : saldoRetenido.getIMP_SDO());
						response.setSDO_AUT(
								(saldoAutorizado.getIMP_SDO().equals(null)) ? "" : saldoAutorizado.getIMP_SDO());
						response.setDISPO((saldoDisponible.getSTD_DEC_15Y2().equals(null)) ? ""
								: saldoDisponible.getSTD_DEC_15Y2());
						response.setSDO_INCID(
								(saldoIncidencias.getIMP_SDO().equals(null)) ? "" : saldoIncidencias.getIMP_SDO());
						response.setSDO_CONECT(
								(saldoConectados.getIMP_SDO().equals(null)) ? "" : saldoConectados.getIMP_SDO());
						response.setBLOQUEOS((bloqueos.getSTD_CHAR_01().equals(null)) ? "" : bloqueos.getSTD_CHAR_01());
						response.setRTRN_CD(
								(codigoRetorno.getRTRN_CD().equals(null)) ? "" : codigoRetorno.getRTRN_CD());
						response.setStatus(1);
						response.setDescripcion("");
						...
						response.setSDO_INCID("");
						response.setSDO_CONECT("");
						response.setBLOQUEOS("");
						response.setRTRN_CD("");
						response.setStatus(-1);
					}
				} else {
					CatalogoErrores catalogo = new CatalogoErrores();
					String mensaje = "";
					nodos.add("ResponseBansefi");
					nodos.add("OTR_CONS_SALDOS_VISTA_TRN");
					catalogo = (CatalogoErrores) util.jsonToObject(catalogo, jsonRes, nodos);
					mensaje = util.limpiarErrores(catalogo.getSTD_TRN_MSJ_PARM_V());
					response.setDescripcion(mensaje);
					response.setStatus(-1);
				}
			} else {
				response.setDescripcion("El servicio de consumo de la regla de la U respondio vacio.");
				response.setStatus(-1);
			}
